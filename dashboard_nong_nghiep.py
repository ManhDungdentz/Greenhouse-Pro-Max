...
st.title("🌿 Hệ Thống Giám Sát Nhà Kính (Bản Full - Link SMS)")
    ...
    if not df.empty:
        ...
        if not df_valid.empty:
            ...
            if "🔴" in status:
                ...
                with c2:
                    # TẠO LINK SMS (Phương án gửi tay nhanh nhất)
                    sms_body = f"CANH BAO VPD: {status}. VPD: {last['VPD']} kPa, Temp: {last['temp']}C, Humi: {last['humi']}%."
                    encoded_msg = urllib.parse.quote(sms_body)
                    sms_link = f"sms:{target_phone}?body={encoded_msg}"
                    st.markdown(f'''
            ...
            st.subheader("📊 Thống kê (Max, Min, Mean)")
