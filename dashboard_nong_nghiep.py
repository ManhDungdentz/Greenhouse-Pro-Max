...
# --- XỬ LÝ DỮ LIỆU & KHỬ NHIỄU (ĐÃ SỬA ĐẢO CỘT VÀ BỎ CHIA 10) ---
def process_data(file):
    ...
    if 'tempKK' in df.columns and 'humiKK' in df.columns:
        df['temp'] = pd.to_numeric(df['humiKK'], errors='coerce') # Nhiệt lấy từ humi
        df['humi'] = pd.to_numeric(df['tempKK'], errors='coerce') # Ẩm lấy từ temp

    # Nếu có cột tiếng Việt (vẫn giữ logic cũ cho file khác nếu cần)
    if 'Nhiệt Độ' in df.columns and df['temp'].isna().all():
        df['temp'] = pd.to_numeric(df['Nhiệt Độ'], errors='coerce') / 10
    if 'Độ ẩm' in df.columns and df['humi'].isna().all():
        df['humi'] = pd.to_numeric(df['Độ ẩm'], errors='coerce') / 10
    ...
    if len(df) > 5:
        for c in ['temp', 'humi']:
            diff = df[c].diff().abs()
            df.loc[diff > 15, c] = np.nan # Tăng ngưỡng khử nhiễu lên 15
...
# --- HÀM VẼ BIỂU ĐỒ (GIỮ NGUYÊN DẢI MÀU) ---
def draw_chart(df_valid, c_info, smooth=True):
    ...
    # --- CÁC DẢI MÀU PHÂN VÙNG (GIỮ NGUYÊN) ---
    ...
    fig.add_hrect(y0=0,      y1=c_lo,   fillcolor="rgba(30, 144, 255, 0.35)", line_width=0, row=1, col=1) # Quá thấp
    fig.add_hrect(y0=c_lo,   y1=c_imin, fillcolor="rgba(255, 165, 0, 0.35)",  line_width=0, row=1, col=1) # Cảnh báo thấp
    fig.add_hrect(y0=c_imin, y1=c_imax, fillcolor="rgba(0, 200, 81, 0.35)",   line_width=0, row=1, col=1) # Lý tưởng
    fig.add_hrect(y0=c_imax, y1=c_wmax, fillcolor="rgba(255, 165, 0, 0.35)",  line_width=0, row=1, col=1) # Cảnh báo cao
    fig.add_hrect(y0=c_wmax, y1=vpd_max, fillcolor="rgba(255, 75, 75, 0.35)",  line_width=0, row=1, col=1) # Quá cao
    ...
    fig.update_layout(height=520, template="plotly_white", hovermode='x unified', margin=dict(l=10, r=10, t=30, b=10))
    return fig
...
if mode == "📂 Xem file JSON":
    ...
    if run_realtime:
        ...
        for i in range(10, 0, -1):
            timer_ph.caption(f"🔄 Cập nhật sau {i} giây.")
            time.sleep(1)
        st.rerun()
