...
# --- CẤU HÌNH GMAIL (ÔNG ĐIỀN VÀO ĐÂY) ---
GMAIL_USER = "your_email@gmail.com" 
GMAIL_PASSWORD = "your_app_password" # Mật khẩu ứng dụng 16 ký tự
RECEIVER_EMAIL = "target_email@gmail.com"

def send_email_alert(vpd, status, temp, humi):
    try:
        msg = MIMEMultipart()
...
# --- 1. TÍNH TOÁN VPD (GIỮ NGUYÊN GỐC) ---
...
# --- 2. XỬ LÝ DỮ LIỆU (GIỮ NGUYÊN + FIX LỖI TIMEZONE) ---
def process_data(file):
    ...
    if 'Thời gian' in df.columns:
        # Fix lỗi mixed timezone để đọc file 13MB
...
# --- 3. GIAO DIỆN CHÍNH (KHÔI PHỤC BẢN GỐC CỦA ÔNG) ---
...
if uploaded_file:
    ...
    if not df.empty:
        ...
        if not df_valid.empty:
            ...
            # --- HIỂN THỊ METRIC (TRẢ LẠI GIAO DIỆN CŨ) ---
            ...
            html_box = f"""<div style="padding:20px; border-radius:10px; background-color:{color}; color:white; text-align:center;">
                <span style="font-size:24px; font-weight:bold;">VPD: {last['VPD']} kPa</span><br>
                <span style="font-size:16px;">{status}</span></div>"""
            col2.markdown(html_box, unsafe_allow_html=True)
            col3.warning(f"**Chỉ đạo vận hành:** {advice}")
            
            # NÚT GỬI MAIL (CHÈN THÊM)
            if "🔴" in status:
                if st.button("📧 Gửi Email Cảnh Báo Ngay"):
                    if send_email_alert(last['VPD'], status, last['temp'], last['humi']):
            ...
            # BẢNG THỐNG KÊ MAX/MIN/MEAN (TRẢ LẠI CHO ÔNG)
            ...
            st.dataframe(
                ...
                .style.map(highlight_vpd, subset=['VPD']),
