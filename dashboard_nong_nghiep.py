...
def draw_chart(df_valid, c_info, smooth=True):
    ...
    fig.add_hrect(y0=0, y1=c_info["low"], fillcolor="rgba(30, 144, 255, 0.3)", line_width=0, row=1, col=1)
    fig.add_hrect(y0=c_info["ideal_min"], y1=c_info["ideal_max"], fillcolor="rgba(0, 200, 81, 0.3)", line_width=0, row=1, col=1)
...
def show_metrics(last, status, advice, color, u_mail, u_pass, t_mail, key_suffix="default"):
...
if mode == "📂 Xem file JSON":
    if uploaded_file:
        ...
        if not df.empty:
            ...
            if not df_valid.empty:
                ...
                show_metrics(last, status, advice, color, u_mail, u_pass, t_mail, key_suffix="file")
    ...
    # --- CHẾ ĐỘ REALTIME (SỬA LỖI DUPLICATE KEY) ---
    if 'rt_history' not in st.session_state:
        st.session_state.rt_history = pd.DataFrame(columns=['Thời gian', 'temp', 'humi', 'VPD', 'STT'])
        # Khởi tạo điểm dữ liệu đầu tiên
        t, h = 28.5, 70.0
        vpd = calculate_vpd(t, h)
        st.session_state.rt_history.loc[0] = [datetime.now().replace(microsecond=0), t, h, vpd, 'SIM-01']

    if run_realtime:
        # Tạo dữ liệu mới dựa trên điểm cũ
        last_t = st.session_state.rt_history.iloc[-1]['temp']
        last_h = st.session_state.rt_history.iloc[-1]['humi']
        new_t = round(np.clip(last_t + np.random.uniform(-0.4, 0.4), 18, 42), 1)
        new_h = round(np.clip(last_h + np.random.uniform(-1.5, 1.5), 30, 95), 1)
        new_vpd = calculate_vpd(new_t, new_h)
        
        new_row = pd.DataFrame([{'Thời gian': datetime.now().replace(microsecond=0), 
                                 'temp': new_t, 'humi': new_h, 'VPD': new_vpd, 'STT': 'SIM-01'}])
    ...
    show_metrics(last_rt, status_rt, advice_rt, color_rt, u_mail, u_pass, t_mail, key_suffix="rt")
    ...
    if run_realtime:
        ...
        for i in range(60, 0, -1):
            placeholder.caption(f"🔄 Cập nhật tiếp theo sau {i} giây...")
            time.sleep(1)
        st.rerun()
    else:
