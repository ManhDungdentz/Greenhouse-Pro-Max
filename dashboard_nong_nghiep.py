...
st.title("🌿 Hệ Thống Giám Sát Nhà Kính (Bản Lọc Nhiễu Triệt Để)")
...
def get_greenhouse_advice(vpd, stage, safe_min, safe_max):
    ...
    if vpd < 0.8: return "🔵 QUÁ THẤP", "Độ ẩm cao!", "#1E90FF"
    if 0.8 <= vpd <= 1.2: return "🟢 LÝ TƯỞNG", "Cây phát triển tốt.", "#00C851"
    if vpd > 1.2: return "🔴 QUÁ CAO", "Stress nhiệt!", "#FF4B4B"
    return "🟡 CẢNH BÁO", "Kiểm tra lại.", "#FFA500"

# --- XỬ LÝ DỮ LIỆU & LỌC NHIỄU MẠNH ---
def process_data(file):
    ...
    # --- THUẬT TOÁN LỌC NHIỄU "CỘT ĐÌNH" ---
    if len(df) > 5:
        for c in ['temp', 'humi']:
            rolling_median = df[c].rolling(window=5, center=True).median()
            # Nếu giá trị lệch quá 5 đơn vị so với trung vị xung quanh -> Coi là nhiễu
            df.loc[(df[c] - rolling_median).abs() > 5, c] = np.nan
...
with st.sidebar:
    ...
    u_mail, u_pass, t_mail = st.text_input("Gmail gửi:"), st.text_input("Mật khẩu:", type="password"), st.text_input("Gmail nhận:")
    st.divider()
...
if uploaded_file:
    ...
    if not df.empty:
        ...
        if filter_mode == "Tháng":
            ...
            start, end = pd.to_datetime(c1.date_input("Từ ngày", df['Thời gian'].min())), pd.to_datetime(c2.date_input("Đến ngày", df['Thời gian'].max())) + timedelta(days=1)
        ...
        st.sidebar.divider()
        def_val = (0.4, 0.8) if "Cây con" in growth_stage else (0.8, 1.2) if "Sinh trưởng" in growth_stage else (1.2, 1.5)
        ...
        if not df_valid.empty:
            ...
            m1.metric("Nhiệt độ", f"{round(last['temp'], 1)} °C"), m1.metric("Độ ẩm", f"{round(last['humi'], 1)} %")
            ...
            m2.markdown(html_box, unsafe_allow_html=True), m3.warning(f"**Chỉ đạo:** {advice}")
            if ("🔴" in status or "🔵" in status) and st.button("📧 Gửi Email"):
                if send_email_alert(u_mail, u_pass, t_mail, last['VPD'], status, last['temp'], last['humi']): st.success("✅ Đã gửi!")
            ...
            fig.update_layout(height=550, margin=dict(l=20, r=20, t=20, b=20), template="plotly_white")
...
else: st.info("👈 Tải file JSON để xem.")
